    ...
    public function exportSummary(Request $request)
    {
        ...
        foreach ($products as $p) {
            ...
            if (!empty($targetBaseNames)) {
                ...
                } else { $p->baseline_name = '-'; $p->baseline_weight = 0; $p->baseline_cost = 0; $p->change_status = '-'; $p->ebd_unit_name = '-'; $p->ebd_density = 0; $p->ebd_pcs_per_pitch = 0; $p->ebd_pcs_per_unit = 0; $p->ebd_net_weight = null; }
            } else {
                ...
                if ($activeBase) {
                    ...
                    if ($predecessor) {
                        $hasDiff = round((float)$activeBase->weight_kg, 4) != round((float)$predecessor->weight_kg, 4) || $activeBase->material_spec_id != $predecessor->material_spec_id || (float)$activeBase->thickness != (float)$activeBase->thickness;
                        $p->change_status = $hasDiff ? 'CHANGE' : 'NO CHANGE';
