...
class ProjectVaveAnalysisController extends Controller
{
    ...
    public function importExcel(Request $request, $isRegular = false)
    {
        ...
        try {
            ...
            $summaryLines = [];
            if ($totalCreated > 0) $summaryLines[] = "<div class='text-emerald-600 font-bold text-[10px]'><i class='fa-solid fa-plus-circle mr-1'></i> {$totalCreated} new versions created</div>";
            if ($totalUpdated > 0) $summaryLines[] = "<div class='text-amber-600 font-bold text-[10px]'><i class='fa-solid fa-pen-to-square mr-1'></i> {$totalUpdated} versions updated</div>";
            if ($unchanged > 0) $summaryLines[] = "<div class='text-gray-500 italic text-[10px]'><i class='fa-solid fa-check-double mr-1'></i> {$unchanged} other versions already up-to-date</div>";
