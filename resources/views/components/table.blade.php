...
<script>
    ...
    window.defaultDataTable = function (selector, userConfig = {}) {
        ...
        const defaults = {
            ...
            dom: "<'flex flex-col sm:flex-row justify-between items-center mb-6 gap-4'<'flex items-center justify-center sm:justify-start gap-3 w-full sm:w-auto'l B><'w-full sm:w-auto flex justify-center sm:justify-end [&_label]:w-full sm:[&_label]:w-auto [&_input]:w-full sm:[&_input]:w-auto'f>>r<'overflow-x-auto w-full relative border border-gray-200 dark:border-gray-700 rounded-xs't><'flex flex-col md:flex-row justify-between items-center gap-4 text-gray-500'i p>",
            buttons: [
