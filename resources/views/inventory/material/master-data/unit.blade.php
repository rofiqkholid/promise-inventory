...
<div class="text-gray-900 dark:text-gray-100">
    {{-- Header Section --}}
    <div class="mb-4">
        <h2 class="text-xl xl:text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tighter leading-none">Unit</h2>
        <p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400 font-normal">Manage all measurement units used in the system.</p>
    </div>

    {{-- UNIFIED CARD HEADER TOOLBAR --}}
    <div id="unitCard" class="mb-0 bg-white dark:bg-gray-800 rounded-t-xs rounded-b-none border border-b-0 border-slate-200 dark:border-gray-700 overflow-hidden shadow-xs">
        <div class="px-5 py-3.5 bg-slate-50/70 dark:bg-slate-900/40 border-b border-slate-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <h3 class="text-xs font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2 tracking-tight">
                <i class="fa-solid fa-ruler-combined text-primary-600"></i> Unit List
            </h3>
            @if(Auth::user()->hasMenuPermission('inventory.master.unit.index', 'create'))
            <button type="button" class="add-button inline-flex items-center justify-center gap-2 px-3.5 h-9 bg-primary-600 hover:bg-primary-700 border border-transparent rounded-xs text-xs font-medium text-white active:scale-[0.98] transition-all shadow-xs w-full sm:w-auto" data-target="unit">
    ...
    <x-table id="unitTable">
        <thead class="bg-gray-50 dark:bg-gray-800/50">
            <tr>
                ...
                @if(Auth::user()->hasMenuPermission('inventory.master.unit.index', 'edit') || Auth::user()->hasMenuPermission('inventory.master.unit.index', 'delete'))
...
{{-- Modals --}}
<div id="modal-unit-add" class="modal-container hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50">
    <div class="relative w-full max-w-md transform overflow-hidden rounded-xs bg-white dark:bg-gray-900 transition-all border border-slate-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-6 py-4 bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">Add Unit</h3>
        ...
        <div class="overflow-y-auto px-6 py-6 custom-scrollbar flex-1">
            <form class="modal-form" data-action="{{ route('inventory.master.unit.store') }}">
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-[11px] font-bold text-slate-600 dark:text-gray-300">Name</label>
...
<div id="modal-unit-edit" class="modal-container hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50">
    <div class="relative w-full max-w-md transform overflow-hidden rounded-xs bg-white dark:bg-gray-900 transition-all border border-slate-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-6 py-4 bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">Edit Unit</h3>
            <button type="button" class="close-modal text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors w-8 h-8 flex items-center justify-center rounded-xs hover:bg-gray-100 dark:hover:bg-gray-800">
                <i class="fa-solid fa-xmark text-lg"></i>
            </button>
        </div>
        ...
        window.masterTable = window.defaultDataTable('#unitTable', {
            ...
            columns: [
                ...
                @if(Auth::user()->hasMenuPermission('inventory.master.unit.index', 'edit') || Auth::user()->hasMenuPermission('inventory.master.unit.index', 'delete'))
                {
                    ...
                    render: (d, t, r) => {
                        let buttons = '';
                        @if(Auth::user()->hasMenuPermission('inventory.master.unit.index', 'edit'))
                        buttons += `
                        ...
                        `;
                        @endif
                        @if(Auth::user()->hasMenuPermission('inventory.master.unit.index', 'delete'))
                        buttons += `
