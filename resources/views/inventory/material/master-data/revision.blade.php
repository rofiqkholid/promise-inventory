...
<div id="modal-revision-add" class="modal-container hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50">
    <div class="relative w-full max-w-md transform overflow-hidden rounded-xs bg-white dark:bg-gray-900 transition-all border border-slate-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-6 py-4 bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="text-sm font-medium text-gray-900 dark:text-white">Add Revision</h3>
        ...
        <div class="overflow-y-auto px-6 py-6 custom-scrollbar flex-1">
            <form class="modal-form" data-action="{{ route('inventory.master.revision.store') }}">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="mb-4">
                        <label class="block mb-2 text-[11px] font-medium text-slate-900 dark:text-gray-300">Revision Code <span class="text-red-500">*</span></label>
                        ...
                        <p class="error-msg hidden text-red-500 text-[10px] mt-1 font-medium"></p>
                    </div>
                    <div class="mb-4">
                        <label class="block mb-2 text-[11px] font-medium text-slate-900 dark:text-gray-300">Sort Order <span class="text-red-500">*</span></label>
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-[11px] font-medium text-slate-900 dark:text-gray-300">Group Name <span class="text-red-500">*</span></label>
...
<div id="modal-revision-edit" class="modal-container hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50">
    <div class="relative w-full max-w-md transform overflow-hidden rounded-xs bg-white dark:bg-gray-900 transition-all border border-slate-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-6 py-4 bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="text-sm font-medium text-gray-900 dark:text-white">Edit Revision</h3>
            <button type="button" class="close-modal text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors w-8 h-8 flex items-center justify-center rounded-xs hover:bg-gray-100 dark:hover:bg-gray-800">
                <i class="fa-solid fa-xmark text-lg"></i>
            </button>
        </div>
    ...
    $(document).ready(function() {
        ...
        window.masterTable = window.defaultDataTable('#revisionTable', {
            ...
            columns: [
                ...
                @if(Auth::user()->hasMenuPermission('inventory.master.revision.index', 'edit') || Auth::user()->hasMenuPermission('inventory.master.revision.index', 'delete'))
                {
                    data: null, orderable: false, searchable: false, className: 'text-center', width: '120px',
                    render: (d, t, r) => {
                        let buttons = '';
                        @if(Auth::user()->hasMenuPermission('inventory.master.revision.index', 'edit'))
                        buttons += `
                        ...
                        `;
                        @endif
                        @if(Auth::user()->hasMenuPermission('inventory.master.revision.index', 'delete'))
                        buttons += `
