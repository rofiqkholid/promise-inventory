...
<div class="p-4 sm:p-6 lg:p-8 text-gray-900 dark:text-gray-100">
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 sm:text-3xl tracking-tighter">Rank</h2>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 font-medium">Manage inventory ranks and their limit values.</p>
        </div>
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form class="modal-form" data-action="{{ route('inventory.master.rank.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="code" required class="bg-white border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-slate-500 focus:border-slate-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white shadow-sm" placeholder="e.g. A">
...
<script>
    $(document).ready(function() {
        ...
        const apiBase = "{{ url('inventory/master/rank') }}";
        let deleteUrl = '';

        window.masterTable = window.defaultDataTable('#rankTable', {
            ajax: { url: '{{ route("inventory.master.rank.data") }}', type: 'GET' },
        ...
        $('.add-button').on('click', function() {
            const $form = $('#modal-rank-add form');
            $form[0].reset(); $form.find('.error-msg').addClass('hidden');
            showMdl('modal-rank-add');
        });

        $(document).on('click', '.edit-btn', function() {
            const id = $(this).data('id');
            $.get(`${apiBase}/${id}`, (data) => {
                const $modal = $('#modal-rank-edit'); 
